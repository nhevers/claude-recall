...
The Worker Service is an Express HTTP server that handles all claude-recall operations. It runs on port 37777 (configurable via `CLAUDE_RECALL_WORKER_PORT`) and is managed by PM2.

## Request Flow

```
Hook (extension/runtime/*-hook.js)
...
- Search operations → MCP server (history-search)
