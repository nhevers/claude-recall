...
const INSTALLED_PATH = path.join(os.homedir(), '.claude', 'plugins', 'marketplaces', 'nhevers');
const CACHE_BASE_PATH = path.join(os.homedir(), '.claude', 'plugins', 'cache', 'nhevers', 'claude-recall');
...
try {
  execSync(
    'rsync -av --delete --exclude=.git --exclude=/.mcp.json ./ ~/.claude/plugins/marketplaces/nhevers/',
    { stdio: 'inherit' }
  );

  console.log('Running npm install in marketplace...');
  execSync(
    'cd ~/.claude/plugins/marketplaces/nhevers/ && npm install',
    { stdio: 'inherit' }
  );
