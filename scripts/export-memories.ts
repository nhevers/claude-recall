 ...
 * Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=claude-recall
 */

import { writeFileSync } from 'fs';
import { homedir } from 'os';
import { join } from 'path';
import { SettingsDefaultsManager } from '../src/common/SettingsDefaultsManager';
import type {
  ObservationRecord,
  SdkSessionRecord,
...
async function exportMemories(query: string, outputFile: string, project?: string) {
  try {
    ...
    const settings = SettingsDefaultsManager.loadFromFile(join(homedir(), '.claude-recall', 'settings.json'));
    const port = parseInt(settings.CLAUDE_RECALL_WORKER_PORT, 10);
...
if (args.length < 2) {
  ...
  console.error('Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=claude-recall');
