...
async function getClaudeRecallVersion(): Promise<string> {
  try {
...
async function getDatabaseInfo(
  ...
  try {
    const dbPath = path.join(dataDir, "claude-recall.db");
...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".claude-recall");
  ...
  const isDevMode = cwd.includes("claude-recall") && !cwd.includes(".claude");

  // Collect version information
  const [ClaudeRecall, claudeCode, bun, osVersion] = await Promise.all([
    getClaudeRecallVersion(),
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "claude-recall.db")),
...
export function formatDiagnostics(diagnostics: SystemDiagnostics): string {
  ...
  output += "## Environment\n\n";
  output += `- **claude-recall**: ${diagnostics.versions.ClaudeRecall}\n`;
  ...
  if (diagnostics.config.settings) {
    ...
    const keySettings = [
      "CLAUDE_RECALL_MODEL",
      "CLAUDE_RECALL_WORKER_PORT",
      "CLAUDE_RECALL_WORKER_HOST",
      "CLAUDE_RECALL_LOG_LEVEL",
      "CLAUDE_RECALL_CONTEXT_OBSERVATIONS",
    ];
