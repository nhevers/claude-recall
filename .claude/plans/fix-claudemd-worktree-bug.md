...
2. Run Claude Code in `~/Scripts/claude-recall/` (main repo)
...
**File: `src/common/paths.ts`** - Path utilities
```typescript
...
- Pattern for `path.join(base, relative)`: Used throughout `src/common/paths.ts`
...
**File: `src/core/worker/agents/ResponseProcessor.ts`**
...
1. [ ] `grep -n "projectRoot" src/core/worker/agents/ResponseProcessor.ts` shows parameter throughout
2. [ ] `grep -n "processAgentResponse" src/core/worker/*.ts` to find all callers
...
**File: `src/core/worker/SDKAgent.ts`**
...
**File: `src/core/worker/GeminiAgent.ts`** - Same pattern
**File: `src/core/worker/OpenRouterAgent.ts`** - Same pattern
...
1. [ ] `grep -n "lastCwd" src/core/worker/SDKAgent.ts` shows tracking
2. [ ] `grep -n "lastCwd" src/core/worker/GeminiAgent.ts` shows tracking
3. [ ] `grep -n "lastCwd" src/core/worker/OpenRouterAgent.ts` shows tracking
4. [ ] `grep -n "processAgentResponse.*lastCwd" src/core/worker/` shows all calls updated
...
grep -n "projectRoot" src/core/worker/agents/ResponseProcessor.ts
grep -n "lastCwd" src/core/worker/SDKAgent.ts

# 2. Confirm path.isAbsolute check added
grep -n "path.isAbsolute" src/utils/claude-md-utils.ts

# 3. Confirm all agents updated
grep -n "processAgentResponse.*lastCwd" src/core/worker/*.ts
...
grep -A 10 "interface ParsedObservation" src/parser/parser.ts | grep cwd
```
...
| `src/core/worker/agents/ResponseProcessor.ts` | Pass `projectRoot` through call chain |
| `src/core/worker/SDKAgent.ts` | Track `lastCwd`, pass to `processAgentResponse` |
| `src/core/worker/GeminiAgent.ts` | Track `lastCwd`, pass to `processAgentResponse` |
| `src/core/worker/OpenRouterAgent.ts` | Track `lastCwd`, pass to `processAgentResponse` |
