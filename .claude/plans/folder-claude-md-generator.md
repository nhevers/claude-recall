...
### Tag Strategy
- Wrap ONLY auto-generated content with `<claude-recall-context>` tags
...
- `<claude-recall-context>` tag is searchable fingerprint for GitHub analytics
...
|----------|----------|---------|
| `findByFile(filePath, options)` | `src/core/sqlite/SessionSearch.ts:342` | Query observations by folder prefix (already supports LIKE wildcards) |
| `updateCursorContextForProject()` | `src/core/integrations/CursorHooksInstaller.ts:98` | Write context files after observation save |
| `writeContextFile()` | `src/utils/cursor-utils.ts:97` | Atomic file write with temp file + rename |
| `extractFirstFile()` | `src/common/timeline-formatting.ts` | Extract file paths from JSON arrays |
| `groupByDate()` | `src/common/timeline-formatting.ts` | Group items chronologically |
| `formatTime()`, `formatDate()` | `src/common/timeline-formatting.ts` | Time formatting |
...
Add ONE new function to `src/core/integrations/CursorHooksInstaller.ts`:
...
4. Write to `<folder>/CLAUDE.md` preserving content outside `<claude-recall-context>` tags
...
- [ ] Preserves content outside `<claude-recall-context>` tags
...
Add call to `updateFolderClaudeMd()` in `src/core/worker/agents/ResponseProcessor.ts`, right after the existing `updateCursorContextForProject()` call at line 266.
...
2. Reuse existing `ResultFormatter.formatSearchResults()` from `src/core/worker/search/ResultFormatter.ts`
...
1. **Manual Test**:
   ...
   - Create a test observation touching `src/core/sqlite/`
   - Verify `src/core/sqlite/CLAUDE.md` is created/updated
   - Verify `<claude-recall-context>` tags are present
