...
**Target:** `src/core/worker-service.ts` and related files
...
- `src/core/worker-service.ts` (813 lines) - Current state
...
**Verification:**
```bash
grep -n "runInteractiveSetup" src/core/worker-service.ts
...
**Verification:**
```bash
grep -n "^import \* as fs" src/core/worker-service.ts
grep -n "import { spawn }" src/core/worker-service.ts
# Expected: No output
npm run build
```
...
} from './adapters/CursorHooksInstaller.js';
```

**Verification:**
```bash
grep "detectClaudeCode\|findCursorHooksDir\|installCursorHooks\|configureCursorMcp" src/core/worker-service.ts
...
**Verification:**
```bash
grep -n "isConfigured" src/core/worker/SDKAgent.ts
# Expected: Method definition
grep -n "setFallbackAgent" src/core/worker-service.ts
...
**Steps:**
1. Update `src/core/worker/agents/ResponseProcessor.ts`:
   - Change: `import { updateCursorContextForProject } from '../../engine-service.js';`
...
**Verification:**
```bash
grep -n "export { updateCursorContextForProject" src/core/worker-service.ts
# Expected: No output
grep -n "updateCursorContextForProject" src/core/worker/agents/ResponseProcessor.ts
...
- `src/core/sync/ChromaSync.ts:126-131`
...
**Verification:**
```bash
grep -n "version: '1.0.0'" src/core/worker-service.ts src/core/sync/ChromaSync.ts
# Expected: No output
npm run build
```
...
grep "import \* as fs from 'fs'" src/core/worker-service.ts
# Expected: No match

# Verify re-export removed
grep "export { updateCursorContextForProject" src/core/worker-service.ts
# Expected: No match

# Verify fallback has check
grep -A2 "setFallbackAgent" src/core/worker-service.ts
