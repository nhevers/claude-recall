...
**File:** `src/core/worker/agents/ResponseProcessor.ts:274-298`
...
- `stripMemoryTagsFromJson()` - removes `<claude-recall-context>` content
...
**Option B:** Create `src/core/folder-index-service.ts` (follows service pattern)
...
Simplify the call site in `src/core/worker/agents/ResponseProcessor.ts`.
...
```typescript
import { updateCursorContextForProject } from '../../engine-service.js';
```
...
Remove the extracted functions from `src/core/integrations/CursorHooksInstaller.ts`.
...
```bash
grep -n "updateFolderClaudeMd\|formatTimelineForClaudeMd\|replaceTaggedContent\|writeFolderClaudeMd" src/core/integrations/CursorHooksInstaller.ts
```
...
### Integration Check
```bash
grep -n "updateFolderClaudeMdFiles" src/core/worker/agents/ResponseProcessor.ts
```

### No Cursor Registry Dependency
```bash
grep -n "readCursorRegistry" src/core/worker/agents/ResponseProcessor.ts
...
2. `src/core/worker/agents/ResponseProcessor.ts` - UPDATE (simplify call site)
3. `src/core/integrations/CursorHooksInstaller.ts` - UPDATE (remove extracted functions)
