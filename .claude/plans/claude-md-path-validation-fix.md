...
claude-recall 9.0's distributed CLAUDE.md feature has a **critical path validation bug** that creates invalid directories when Claude SDK agent outputs non-path strings in file tracking XML tags (`<files_read>`, `<files_modified>`).
...
- `path.isAbsolute('~/.claude-recall/logs')` returns `false` (Node.js doesn't recognize `~`)
- Code joins: `path.join(projectRoot, '~/.claude-recall/logs')` → `/project/~/.claude-recall/logs`
...
./PR #610 on nhevers/          ← GitHub PR reference
...
| `PR #610 on nhevers/` | Invalid PR reference directory |
