...
**Exit Code Constants** - `src/common/hook-constants.ts:18-23`:
```typescript
...
**Timeout Constants** - `src/common/hook-constants.ts:1-8`:
```typescript
...
**Platform Timeout Function** - `src/core/infrastructure/ProcessManager.ts:70-73`:
```typescript
...
| File | Change | Lines |
|------|--------|-------|
| `src/core/infrastructure/ProcessManager.ts` | Add POWERSHELL_TIMEOUT constant, reduce from 60000 to 10000 | 93, 123, 175, 241 |
| `src/common/hook-constants.ts` | Add POWERSHELL_TIMEOUT constant | After line 8 |
...
- DO NOT create migration guide for Cursor (shell scripts still exist in integrations/cursor/, not removed)
...
1. Copy constant pattern from `src/common/hook-constants.ts:1-8`
2. Copy usage pattern from `src/core/infrastructure/ProcessManager.ts:93`
...
- [ ] `grep -n "60000" src/core/infrastructure/ProcessManager.ts` returns 0 matches
- [ ] `grep -n "POWERSHELL_COMMAND" src/core/infrastructure/ProcessManager.ts` returns 4 matches
...
1. **Add section after "File Locations"** in `/Users/alexnewman/Scripts/claude-recall/CLAUDE.md`:
...
3. **No magic numbers remain**: `grep -rn "60000" src/core/infrastructure/ProcessManager.ts` returns 0
