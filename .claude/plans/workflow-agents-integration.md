# Plan: Integrate Workflow Agents and Commands into claude-recall

## Executive Summary

This plan integrates the `/make-plan` and `/do` orchestration workflow from `~/.claude/commands/` into the claude-recall plugin as project-level development tools.
...
- Existing example: `.claude/skills/version-bump/SKILL.md` for claude-recall project tools
...
This directory contains slash commands **for developing and maintaining the claude-recall project itself**.
...
Add project-level orchestration commands for claude-recall development:
