...
**src/core/worker/agents/ResponseProcessor.ts**
...
1. Add imports from shared utilities:
   ```typescript
   import { formatDate, formatTime, extractFirstFile, estimateTokens, groupByDate } from '../common/timeline-formatting.js';
...
**Pattern to Copy From:** `src/core/worker/search/ResultFormatter.ts` lines 56-108

**Key APIs:**
- `groupByDate(items, getDate)` - from `src/common/timeline-formatting.ts:104-127`
- `formatTime(epoch)` - from `src/common/timeline-formatting.ts:46-53`
- `formatDate(epoch)` - from `src/common/timeline-formatting.ts:59-66`
- `extractFirstFile(filesModified, cwd)` - from `src/common/timeline-formatting.ts:81-84`
- `estimateTokens(text)` - from `src/common/timeline-formatting.ts:89-92`
- `ModeManager.getInstance().getTypeIcon(type)` - from `src/core/domain/ModeManager.ts`
...
- Read `src/core/worker/http/routes/SearchRoutes.ts` to see by-file response format
...
| File | Pattern |
|------|---------|
| `src/core/worker/search/ResultFormatter.ts:56-108` | Date/file grouping logic |
| `src/common/timeline-formatting.ts` | All formatting utilities |
| `src/core/domain/ModeManager.ts` | Type icon lookup |
