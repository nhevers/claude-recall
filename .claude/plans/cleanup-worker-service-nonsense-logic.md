...
**Target:** `src/core/worker-service.ts` (813 lines)
...
**Evidence:** `grep -n "runInteractiveSetup" src/core/worker-service.ts` returns only definition
...
1. After deleting `runInteractiveSetup`, grep for remaining usages:
   - `grep "homedir" src/core/worker-service.ts`
   - `grep "readline" src/core/worker-service.ts`
...
**Verification:**
- `grep "detectClaudeCode\|findCursorHooksDir\|installCursorHooks\|configureCursorMcp" src/core/worker-service.ts` returns nothing
- Build succeeds
...
**Steps:**
1. Grep `onRestart` in `src/core/server/`
...
grep "import { spawn }" src/core/worker-service.ts
```
Expected: No matches
